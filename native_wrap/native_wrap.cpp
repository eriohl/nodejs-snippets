...
public:
  static void Init(v8::Isolate* isolate)
  {
    // Prepare template.
    //
    // We create two internal fields: One for the tracker (field 0) and one for
    // the handle to the "third party" object (field 1).
    
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, 0);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "ThirdParty"));
  ...
  static v8::Local<v8::Object> NewInstance(
    v8::Isolate *isolate, const double value
    )
  {
    ...
    v8::Local<v8::Object> handle = tpl->InstanceTemplate()->NewInstance();
    
    // Store the handle of the "third party" object in an internal field of the
    // V8 object.
    
    handle->SetAlignedPointerInInternalField(1, obj);
    
    // Create a tracker that destroys the third party object once it goes out of
    // scope.
  ...
  static bool IsInstanceOrThrow(
    v8::Isolate *isolate, v8::Handle<v8::Object> &handle
    )
  {
    if (!handle.IsEmpty())
    {
      v8::Handle<v8::Value> target_prototype = handle->GetPrototype();
...
private:
  ...
  public:
    ...
    static void New(third_party::handle_t obj, v8::Local<v8::Object> &handle)
    {
      ...
      // handle referencing the V8 object. When all non-weak handled referecing
      // the object goes out of scope a callback registered with weak handle is
      // called which in turn deletes it. This is how the destructor of the
      // native object is called.
  ...
  static void PlusOne(const v8::FunctionCallbackInfo<v8::Value>& args)
  {
    ...
    args.GetReturnValue().Set(
      v8::Number::New(isolate, PlusOne(this_handle))
      );
  }
...
static void PlusOne(const v8::FunctionCallbackInfo<v8::Value>& args)
{
  ...
  v8::Local<v8::Object> arg_handle = args[0]->ToObject();  
  if (!ThirdParty::IsInstanceOrThrow(isolate, arg_handle)) return;
  
  args.GetReturnValue().Set(
    v8::Number::New(isolate, ThirdParty::PlusOne(arg_handle))
    );
}
